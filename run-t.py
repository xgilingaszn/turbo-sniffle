...
def run_bot(data_account, recover=1):
    ...
    try:
        ...
        lasturl = kw.replace("~ Update Pict"," ")
        nama_modif = lasturl.replace(" ", "-")
        gmail = f"wongcosny-{random_string(6)}@gmail.com"
        slug = f"{kw}-update-{random_string(6)}"
        judul = f"{kw}"
        link = f"https://clipsfans.com/bento/?title= CLICK HERE >> {kw} ?ref=bentox"
        ...
        try:
            konten = f'{kw} video full update pict & video '
            driver.find_element(
                By.XPATH,
